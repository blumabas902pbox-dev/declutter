...
function updateEmotionalStatus() {
    ...
    if (latestTask.includes("win") || latestTask.includes("can") || latestTask.includes("party") || latestTask.includes("dance") || latestTask.includes("sing")) {
        statusDisplay.innerText = "STATUS: ECSTATIC";
    } else if (latestTask.includes("risk") || latestTask.includes("brave") || latestTask.includes("speak") || latestTask.includes("end") || latestTask.includes("must")) {
        statusDisplay.innerText = "STATUS: FEARLESS";
    } else if (latestTask.includes("thank") || latestTask.includes("kind") || latestTask.includes("family") || latestTask.includes("regards") || latestTask.includes("owe")) {
        statusDisplay.innerText = "STATUS: GRATEFUL";
    } else if (latestTask.includes("believe") || latestTask.includes("learn") || latestTask.includes("dream") || latestTask.includes("pursue") || latestTask.includes("try")) {
        statusDisplay.innerText = "STATUS: INSPIRED";
    } else if (latestTask.includes("vacation") || latestTask.includes("sleep") || latestTask.includes("read") || latestTask.includes("listen") || latestTask.includes("watching")) {
        statusDisplay.innerText = "STATUS: RELAXED";
    } 

    else if (latestTask.includes("tired") || latestTask.includes("more") || latestTask.includes("heavy") || latestTask.includes("can't") || latestTask.includes("unfinished")) {
        statusDisplay.innerText = "STATUS: EXHAUSTED";
    } else if (latestTask.includes("hurts") || latestTask.includes("closure") || latestTask.includes("stuck") || latestTask.includes("not") || latestTask.includes("stress")) {
        statusDisplay.innerText = "STATUS: HEARTBROKEN";
    } else if (latestTask.includes("don't") || latestTask.includes("am i") || latestTask.includes("at me") || latestTask.includes("feeling") || latestTask.includes("did i")) {
        statusDisplay.innerText = "STATUS: INSECURE";
    } else if (latestTask.includes("remember") || latestTask.includes("old") || latestTask.includes("back") || latestTask.includes("then") || latestTask.includes("miss")) {
        statusDisplay.innerText = "STATUS: NOSTALGIC";
    } else if (latestTask.includes("need") || latestTask.includes("many") || ("where") || latestTask.includes("do I") || latestTask.includes("urgent") || latestTask.includes("much")) {
        statusDisplay.innerText = "STATUS: OVERWHELMED";
    }

    else if (total > 15) {
        statusDisplay.innerText = "STATUS: OVERWHELMED";
    } else if (total === 0 && deletedCount > 0) {
        statusDisplay.innerText = "STATUS: EXHAUSTED";
...
function sweepCompleted() {
    ...
    if (completedTasks.length === 0) return alert("Nothing to sweep. Select or add your task first!");
